...
def train(args, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            loss = outputs[0]
            epoch_iterator.set_description("loss:{:.2f},eval acc:{:.2f}".format(loss, eval_best_acc), refresh=False)
            #logger.info(f"loss {loss} \n eval acc: {eval_best_acc}")
            if args.n_gpu > 1:
                loss = loss.mean()
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.eval_steps > 0 and global_step % args.eval_steps == 0:
                    if args.evaluate_during_training:
                        ...
                        logger.info(f"Evaluation Acc: {acc} at step:{global_step}")
                        eval_best_acc = max(eval_best_acc, acc)

                if args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    tb_writer.add_scalar('lr', scheduler.get_lr()[0], global_step)
                    tb_writer.add_scalar('loss', (tr_loss - logging_loss) / args.logging_steps, global_step)
                    #logger.info(f"learning rate: {scheduler.get_lr()[0]:.2f} at step {global_step}")
                    #logger.info(f"Loss: {(tr_loss - logging_loss) / args.logging_steps :.2f} at step : {global_step}")
